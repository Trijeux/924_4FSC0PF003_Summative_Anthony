...
{
	std::cout << "_____________NO_LOOP_ONE_____________" << std::endl;
	bool   check_conditions = false;
	bool   action_executed = false;
	NoLoop no_loop;

	no_loop.Add(std::make_unique<Leaf>([&]
	{
		return simple_action(check_conditions, action_executed);
	}));
...
{
	std::cout << "_____________NO_LOOP_MUTI_____________" << std::endl;
	bool   check_conditions = false;
	bool   action_executed = false;
	...
	no_loop.Add(std::make_unique<Leaf>([&]
	{
		return simple_action(check_conditions2, action_executed2);
	}));
...
{
	std::cout << "_____________SELECTOR_ONE_____________" << std::endl;

	bool   check_conditions = false;
	bool   action_executed = false;
	Selector selector;

	selector.Add(std::make_unique<Leaf>([&]
	{
		return simple_action(check_conditions, action_executed);
	}));
...
{
	std::cout << "_____________SELECTOR_MUTI_____________" << std::endl;
